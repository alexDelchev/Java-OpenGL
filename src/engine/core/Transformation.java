...
public class Transformation {
	...
	public Matrix4f buildOrthoProjectionModelMatrix(GameItem gameItem, Matrix4f orthoMatrix){
		...
		Matrix4f orthoMatrixCurrent = new Matrix4f(orthoMatrix);
		orthoMatrixCurrent.mul(modelMatrix);
		
		return orthoMatrixCurrent;
	}
	...
	public Matrix4f buildModelWorldMatrix(GameItem item){
		
		Vector3f rotation = item.getRotation();
		
		Matrix4f modelWorldMatrix = new Matrix4f();
		modelWorldMatrix.identity().translate(item.getPosition())
		                           .rotateX((float) Math.toRadians(-rotation.x))
		                           .rotateY((float) Math.toRadians(-rotation.y))
		                           .rotateZ((float) Math.toRadians(-rotation.z))
		                           .scale(item.getScale());
